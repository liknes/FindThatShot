...
- **White bars inside the in-app player — fixed structurally by sizing the player area to the video's aspect ratio.** Multiple iterations of Win32 interop fixes (class-brush patch via `SetClassLongPtr(GCLP_HBRBACKGROUND, BLACK_BRUSH)`, `WM_PARENTNOTIFY/WM_CREATE` hook for new vout windows, `WM_ERASEBKGND` subclass via `SetWindowSubclass`, aggressive `RedrawWindow(RDW_INVALIDATE|RDW_ERASE|RDW_ALLCHILDREN|RDW_UPDATENOW)`, swapping the LibVLC vout backend to `--vout=direct3d9`) all targeted *colouring* the letterbox. None of them worked: VLC's default Direct3D11 vout renders straight to a DXGI swap chain, the GDI tricks never reach those pixels, and even the D3D9 backend didn't reliably clear the surround. The actual fix is to make letterbox impossible: a tiny `AspectRatioPanel` (in `Helpers/Controls/`) sizes the `vlc:VideoView` to exactly the current video's native aspect ratio inside the existing black `Border`. The video then fills 100% of the VLC render surface — there are no letterbox pixels for VLC to mis-paint, and the strip of empty space around the picture is ordinary WPF, painted by the parent `Border`'s black background. The panel's `Ratio` is bound to `VideoItemViewModel.AspectRatio` (computed `Width/Height` from the metadata, falls back to 16:9 when dimensions are missing), so the player adapts per-clip. As part of this fix all of the prior interop scaffolding was removed: deleted `Helpers/Interop/HwndHostBackgroundFix.cs` (~290 lines of P/Invoke and subclassing), removed every `HwndHostBackgroundFix.Apply` call site from `MainWindow.xaml.cs` (Loaded handler, `ApplyReviewModeLayout`'s deferred dispatcher invocation, `MediaPlayer.Vout` event handler), removed the `MediaPlayer.Vout` subscription and unsubscription, and reverted `App.xaml.cs` from `new LibVLC("--vout=direct3d9")` back to the default `new LibVLC()`. We also briefly attempted to force a dark `Window.Background` on every window as a "structural" fix earlier in this saga; that was based on a wrong assumption (ModernWPF's `WindowHelper.UseModernWindowStyle` already applies a theme-correct background) and it shifted the overall tint of the catalog, sidebar and editor panes, so it was reverted at the time.
